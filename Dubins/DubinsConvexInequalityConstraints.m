%   DubinsConvexInequalityConstraints.m: Computes the values for convex inequality
%   constraints
%
...
%
%   Outputs:        g_convineq           Value of the inequality constraints 
%
%   Note:           This function assumes the convention g(x) <= 0
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function g_convineq = DubinsConvexInequalityConstraints(vars, dubprob) 
...
% Consolidate
g_convineq = [g_control; g_time];

% State Constraints (Environmental Bounds
if ~isinf(dubprob.environment.non_dim.xbounds(1))
    g_convineq = [g_convineq; dubprob.environment.non_dim.xbounds(1) - vars(range.x)];
end
if ~isinf(dubprob.environment.non_dim.ybounds(1))
    g_convineq = [g_convineq; dubprob.environment.non_dim.ybounds(1) - vars(range.y)];
end
if ~isinf(dubprob.environment.non_dim.xbounds(2))
    g_convineq = [g_convineq; vars(range.x) - dubprob.environment.non_dim.xbounds(2)];
end
if ~isinf(dubprob.environment.non_dim.ybounds(2))
    g_convineq = [g_convineq; vars(range.y) - dubprob.environment.non_dim.ybounds(2)];
end
end