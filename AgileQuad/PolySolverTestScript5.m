...
alphaT = 0.01;   % backtracking parameter
betaT = 0.8;    % backtracking parameter
keys = [ ...    % keypoints or waypoints for trajectory 
    xcen, ycen+0.5, zcen, 0;...
    xcen+0.5, ycen, zcen, 0;...
    xcen, ycen-0.6, zcen, 0;...
%     0, -1, 3, 0;...
%     1, 0, 4, 0;...
%     2, 1, 5, 0;...
%     3, 0, 6, 0;...
...
h_step = 1e-3;  % step size for numerical differentiation
lowVel = 0.1;   % [m/s] low velocity for timing estimate
T0 = 0;         % [s] initial time
plotRes = true;
...
J{iter} = trace(p_coefs{iter}'*Q*p_coefs{iter}) + wT*ones(1,nSeg)*Tdel{1};
...
for l = 1:nSeg
%     baseInd = (l-1)*nCoef;
    stepVec = zeros(nSeg,1);
    stepVec(l) = h_step*Tdel{iter}(l,1);
    Ttemp = Tdel{iter}+stepVec;
    [Qtemp, Atemp, ~, ~, ~, ~] = AgileQuadFormPolyMats( polytrajinfo, Ttemp );
    p_temp = AgileQuadSolvePolynomial( polytrajinfo, Qtemp, Atemp, d, d_fixed,...
        orderMat, matchMat_A, matchMat_d);
%     Jtemp = p_temp(:,1)'*Qtemp*p_temp(:,1) + wT*ones(1,nSeg)*Ttemp;
    Jtemp = trace(p_temp'*Qtemp*p_temp) + wT*ones(1,nSeg)*Ttemp;
    gradJ(l,1) =  (Jtemp - J{iter})/h_step;
end
gradJnorm2 = gradJ'*gradJ;
clear l baseInd p_l stepVec Ttemp Qtemp Atemp p_temp

% Optimize over time-weighted cost function using gradient descent
delJ = etaDescent+1;
while gradJnorm2 > etaDescent && delJ > etaDescent
    ...
    if ~all(Tnext)
        ...
        [Qnext, Anext, ~, ~, ~, ~] = AgileQuadFormPolyMats(polytrajinfo, Tnext);
        p_next = AgileQuadSolvePolynomial( polytrajinfo, Qnext, Anext, d, d_fixed, orderMat, ...
                matchMat_A, matchMat_d);
%         Jnext = p_next(:,1)'*Qnext*p_next(:,1) + wT*ones(1,nSeg)*Tnext;
        Jnext = trace(p_next'*Qnext*p_next) + wT*ones(1,nSeg)*Tnext;
