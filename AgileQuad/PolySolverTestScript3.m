% PolySolverTestScript3.m is a script to develop and test a solver for polynomial trajectories
%
%	AUTHOR: Ross Allen
%	DATE:	Feb 17, 2015
%
    ...
    for k = 0:N                 % order of derivative
        ...
        for i = k:N
            ...
            for j = k:N
                ...
                gradJMat{l}(i+1,j+1,kk) = tempProd*Tdel(l,1)^(i+j-2*k); 
            end
        end
    end
...
for l = 1:nSeg
    ...
    gradJ(l,1) =  p_l'*gradJMat{l}*p_l +wT; 
end
%gradJ(nSeg,1) = p_l1'*gradJMat{nSeg}*p_l1 + wT;
Tnew = Tdel -  gamT*gradJ;
