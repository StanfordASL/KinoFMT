%   DSSCTrainSVM.m: Compute final time for objective function 
%
...
%   
%   Notes:
%       - THIS COULD BE GENERALIZED TO A SYSTEM-AGNOSTIC FUNCTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [mpinfo] = DSSCTrainNeighborClassifier(mpinfo)
...
elseif nMLTrainingSamples <= 0 || nMLTrainingSamples > n2PBVPs
    ...
    disp('Prematurely exiting DSSCTrainNeigborClassifier...')
    return
...
[svm_output, n_training_nonclassifications, n_training_errors, percent_training_errors, ...
    ...
    DSSCTrainSVM(stateMat, evalMat, costMat,...
